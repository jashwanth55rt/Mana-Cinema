...
class MovieViewModel(application: Application) : AndroidViewModel(application) {
    ...
    init {
        ...
        viewModelScope.launch {
            ...
            // Sync or read cached items (filtering out any old seed data)
            launch {
                repository.localMoviesFlow.collect { entities ->
                    val filtered = entities.filter { !it.id.startsWith("seed_") }
                    if (filtered.isNotEmpty()) {
                        _movies.value = filtered.map {
                            Movie(
                                id = it.id,
                                title = it.title,
                                poster = it.poster,
            ...
            launch {
                repository.localSeriesFlow.collect { entities ->
                    val filtered = entities.filter { !it.id.startsWith("seed_") }
                    if (_webSeries.value.isEmpty() && filtered.isNotEmpty()) {
                        _webSeries.value = filtered.map { s ->
